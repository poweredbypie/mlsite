...
if (process.env.BOT_TOKEN === undefined || process.env.MONGODB_URI === undefined) env.config()
...
const transaction = (fn: (req: Request, res: Response, session: ClientSession) => Promise<number>) => {
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      result = typeof code === 'number' ? res.sendStatus(code) : console.log(code)
    } finally {
...
app.get('/levels/:name', async (req, res) => {
  const level = await Level.findOne({ name: req.params.name })
    ...
    .select('-_id -__v')
  return level ? res.status(200).json(level) : res.status(404).send('Level not found.')
})
...
app.get('/players/:name', async (req, res) => {
  const player = await Player.findOne({ name: req.params.name })
    ...
    .select('-_id -id -__v')
  return player ? res.status(200).json(player) : res.status(404).send('Player not found.')
})
...
app.post(
  ...
  transaction(async (req, res, session) => {
    ...
    const player = new Player({
      name: req.body.name as string,
      points: 0,
      discord: req.body.discord === null ? undefined : (req.body.discord as string),
    })
...
app.delete(
  ...
  authed,
  transaction(async (req, res, session) => {
    if (req.body.player === undefined || req.body.level === undefined) throw 400
